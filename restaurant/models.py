...
def generate_restaurant_code():
    ...
    if Restaurant.objects.filter(referance=code).exists():
        return generate_restaurant_code
    return code

class Restaurant(CommonFields):
    id = models.BigAutoField(primary_key = True)
    referance = models.CharField(max_length=300,default=generate_restaurant_code)
...
class RestaurantTable(CommonFields):
