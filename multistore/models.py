...
class BusinessEntity(CommonFields):
    ...
    owner = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="owner", null=True, blank=True)
...
class Outlet(CommonFields):
    ...
    outlet_code = models.CharField(
        max_length=100, default=generate_outlet_code)
    description = models.TextField(default='', null=True, blank=True)
    businessentity = models.ForeignKey(
        BusinessEntity, on_delete=models.CASCADE, null=True, blank=True)
    logo = models.ImageField(upload_to="logo/", null=True, blank=True)
    phone_number = models.CharField(max_length=100, null=True)
    qr_code = models.ImageField(upload_to="qr_code/", default="", null=True)
    address = models.CharField(max_length=100, null=True)
    owner = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="user_owner", null=True)
...
class QrSingature(CommonFields):
    outlet = models.ForeignKey(
        Outlet, on_delete=models.CASCADE, related_name="outlet", null=True, blank=True)
    table_name = models.CharField(max_length=20, null=True, blank=True)
    qr_code = models.FileField(
        default="", upload_to="qrsingature/", null=True, blank=True)
    description = models.TextField(default='', null=True, blank=True)
...
class OrderSession(CommonFields):
    restaurant_table = models.ForeignKey(
        QrSingature, on_delete=models.CASCADE, related_name='qrsingature_table')
